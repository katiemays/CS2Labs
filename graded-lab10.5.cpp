...
class Triangle : public Figure
{
  public:
     //constructor
     //sets base and symbol to a standard precedent, as well as x and y coordinates
     ...
     void Display(bool erase=false)
     {
		...
		int x = get_x();
		int y = get_y();
		int base = side_length;

		//fill str with a line of char c
		//if erase is true, fills line with spaces 
		//(to erase current drawing)
		...
		{
			str[base]='\0'; //terminating character 
			move (y+j,x); //move cursor to (x, y+j) 
			printw (str);  //display str in the current pos
			base--;
		}
